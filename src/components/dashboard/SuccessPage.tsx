...
const SuccessPage = () => {
  ...
  const fetchSuccesses = async (uid: string) => {
    ...
    // Get display names for all unique user_ids
    ...
    if (userIds.length > 0) {
      ...
      if (profiles) {
        profileMap = Object.fromEntries(profiles.map(p => [p.user_id, p.display_name || "Anonyme"]));
      }
    }
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const { error: insertError } = await supabase
        .from("user_successes")
        .insert({ user_id: userId, image_path: filePath });
    ...
    if (uploadedCount > 0) {
      toast.success(`${uploadedCount} succès uploadé${uploadedCount > 1 ? "s" : ""} !`);
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="max-w-3xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div className="border border-border bg-card rounded-lg p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-mono text-muted-foreground">
              {myCount} succès uploadé{myCount !== 1 ? "s" : ""}
            </span>
          </div>
          ...
          {nextMilestone && (
            <p className="text-xs text-muted-foreground">
              Prochain palier : <span className="font-semibold text-foreground">{nextMilestone.label}</span>
              {" "}— encore {nextMilestone.count - myCount} succès
            </p>
          )}
        ...
        {/* Upload zone */}
        <div className="border border-dashed border-border rounded-lg p-6 text-center space-y-3 bg-card hover:border-primary/40 transition-colors">
